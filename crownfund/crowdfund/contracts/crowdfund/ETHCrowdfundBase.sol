...
abstract contract ETHCrowdfundBase is Implementation {
    ...
    function getCrowdfundLifecycle() public view returns (CrowdfundLifecycle lifecycle) {
        ...
        if (block.timestamp < expiry_) {
            if (totalContributions == maxTotalContributions ||
                minContribution > maxTotalContributions - totalContributions) {
    ...
    function sendFundingSplit() public virtual returns (uint96 splitAmount) {
