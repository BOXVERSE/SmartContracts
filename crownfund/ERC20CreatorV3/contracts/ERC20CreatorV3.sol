...
contract ERC20CreatorV3 is IERC721Receiver {
    ...
    /// @notice Mapping to track LP token IDs by ERC20 token address
    mapping(address => uint256) public tokenToLpTokenId;
    /// @notice Mapping to track ITokenDistributor.DistributionInfo
    mapping(address => ITokenDistributor.DistributionInfo) public tokenToTokenDistributionInfo;
    ...
    /// @param fundingSplitRecipient The fundingSplitRecipient which the remaining ETH will be sent to
    /// @param lpFeeRecipient The lpFeeRecipient which the LP fee will be sent to
    ...
    function createToken(
        ...
        if (config.numTokensForDistribution > 0) {
            ...
            ITokenDistributor.DistributionInfo memory distributionInfo = TOKEN_DISTRIBUTOR.createErc20Distribution(
                token,
                Party(payable(party)),
                payable(address(0)),
                0
            );
            tokenToTokenDistributionInfo[address(token)] = distributionInfo;
        }
        ...
        // Store the LP token ID for reference
        tokenToLpTokenId[address(token)] = lpTokenId;
        ...
        // Transfer remaining ETH to the fundingSplitRecipient
        if (address(this).balance > 0) {
            payable(fundingSplitRecipient).call{ value: address(this).balance, gas: 100_000 }("");
        }
    ...
    /// @notice Get the LP token ID for a specific ERC20 token
    /// @param tokenAddress The address of the ERC20 token
    /// @return lpTokenId The ID of the LP position NFT in Uniswap V3
    function getLpTokenId(address tokenAddress) external view returns (uint256) {
        uint256 lpTokenId = tokenToLpTokenId[tokenAddress];
        require(lpTokenId != 0, "LP position not found for this token");

        return lpTokenId;
    }

    /// @notice Get the DistributionInfo for a specific ERC20 token
    /// @param tokenAddress The address of the ERC20 token
    /// @return ITokenDistributor.DistributionInfo The distribution info
    function getTokenDistributionInfo(address tokenAddress) external view returns(ITokenDistributor.DistributionInfo memory) {
        ITokenDistributor.DistributionInfo memory distributionInfo = tokenToTokenDistributionInfo[tokenAddress];
        require(distributionInfo.token != address(0), "TokenDistributionInfo not found for this token");
